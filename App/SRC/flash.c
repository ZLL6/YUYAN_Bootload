...
void STMFLASH_Write(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)	
{
    ...
    secpos=offaddr/STM_SECTOR_SIZE;			//扇区地址  0~127 for STM32F103RBT6  //11
    secoff=(offaddr%STM_SECTOR_SIZE)/2;		//在扇区内的偏移(2个字节为基本单位.)  //510
    secremain=STM_SECTOR_SIZE/2-secoff;		//扇区剩余空间大小   // 2
...
{
	  if(STMFLASH_ReadHalfWord(WriteAddr) == WriteData)
		return;
